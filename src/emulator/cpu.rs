//! CPU PS5 Otimizada com Cache, Pipeline e JIT
...
impl CpuCore {
    ...
    pub fn step(&mut self, memory: &mut VirtualMemory) -> Result<(), String> {
        if self.halted || self.status == CoreStatus::Halted {
            return Ok(());
        }
...
impl Ps5Cpu {
    ...
    pub fn run(&mut self, memory: &mut VirtualMemory, max_instructions: u64) -> Result<(), String> {
        ...
        while executed < max_instructions {
            ...
            let all_halted = self.cores.iter().all(|c| c.halted || c.status == CoreStatus::Halted);
            if all_halted {
                break;
            }
    ...
    pub fn get_stats(&self) -> String {
        ...
        let ipc = if self.total_cycles > 0 { 
            self.total_instructions as f32 / self.total_cycles as f32 
        } else { 0.0 };
